...
LABEL_PATH = 'labels/bathroom_labels.csv'
...
class CustomDataset(Dataset):
    def __init__(self, csv_file, train_test, label_name,transform=None):
        ...
        class_names = sorted(list(df[label_name].unique()))
...
image_datasets = {x: CustomDataset(LABEL_PATH, train_test=x, label_name=TARGET_NAME,transform=data_transforms[x]) for x in ['train', 'val']}
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=32, shuffle=True, num_workers=4) for x in ['train', 'val']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}

class_names = sorted(list(labels[TARGET_NAME].unique()))
...
def train(class_names, epochs, dataloaders):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model, "./models/bathroom_best_loss_model.pth")
        
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model, "./models/bathroom_best_acc_model.pth")
