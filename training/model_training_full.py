...
LABEL_PATH = 'labels/allin_labels.csv'
...
class CustomDataset(Dataset):
    def __init__(self, csv_file, train_test, label_name,transform=None):
        ...
        df['full_path'] = df['path'].apply(lambda x: os.path.join(os.getcwd(),'data',x))
...
image_datasets = {x: CustomDataset(LABEL_PATH, train_test=x, label_name='roomType',transform=data_transforms[x]) for x in ['train', 'val']}
...
def train(class_names, epochs, dataloaders):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model, "./models/allin_best_loss_model.pth")
        
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model, "./models/allin_best_acc_model.pth")
